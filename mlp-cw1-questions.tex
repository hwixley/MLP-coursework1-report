...
\youranswer{
%WORKKK
From these results it is evident that for the EMNIST dataset the Dropout, L1 regularisation, and L2 regularisation techniques are all very useful for mitigating overfitting. From figure~\ref{fig:dropoutrates} we can see that for $0.7 <= p <= 0.95$ (where $p$ is the Dropout probability) that $p$ is directly proportional to validation accuracy and generalisation gap, and is optimal at a value of 0.95. From figure~\ref{fig:weightrates} we can see that for $1e-1 <= \beta <= 1e-4$ (where $\beta$ is the regularisation parameter) that $\beta$ is inversely proportional to generalisation gap and validation accuracy, 
From figure~\ref{fig:extra} we can see that for $p = 0.95$ and $1e-6 <= \beta <= 1e-4$: L2 regularisation is the better weight penalty technique given it produces the best validation accuracies across all values of $\beta$, $\beta = 1e-5$ is the optimal parameter setting in this context for both L1 and L2 regularisation, and thus the parameters for the best performing model was $\beta = 1e-5$ for L2 regularisation and $p = 0.95$ for Dropout.
...
To optimise the random seed we would just iterate through various random seeds to find which produces the optimal start point for the weights and Dropout layers. This optimisation is particularly important in the context of gradient descent due to the fact we want to find the global minimum of the error function. This is due to the fact that performing gradient descent will always bring us to a local minimum but not necessarily a global minimum as it finds the minimum closest to the initialised weights, and cannot tell which minimum is local and which is global. Thus finding this global minimum is fully dependent on the random seed as this seed denotes which minimum our model starts closest to.
